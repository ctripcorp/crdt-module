...
int sscanCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc)
{
    ...
    if (parseScanCursorOrReply(ctx, argv[2], &cursor) == CRDT_ERROR)
        return 0;
                             ...
                             RedisModuleString *dstkey, int op)
{
    ...
    {
        int replyed = 0;
        RedisModuleKey *moduleKey = dstkey ? getRedisModuleKey(ctx, setkeys[j], CrdtSet, REDISMODULE_WRITE, NULL) : getRedisModuleKey(ctx, setkeys[j], CrdtSet, REDISMODULE_READ, &replyed);
        if (moduleKey == NULL)
        {
            if (replyed)
            {
    ...
    else if (op == SET_OP_DIFF && target_sets[0] && diff_algo == 1)
    {
        ...
        while ((de = dictNext(di)) != NULL)
        {
            ...
            for (j = 1; j < setnum; j++)
            {
                ...
                if (target_sets[j] == target_sets[0])
                    break; /* same set! */
                if (dictFind(getSetDict(target_sets[j]), ele))
                    break;
            }
            if (j == setnum)
            {
...
{
    size_t cmdlen = 0;
    if (crdt_sadd_head_len == 0)
        crdt_sadd_head_len = strlen(crdt_sadd_head);
